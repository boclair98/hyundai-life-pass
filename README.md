...
로그인 없이 `충전`과 `내 차 케어`의 위치 기반 탐색을 먼저 사용할 수 있습니다. 현대자동차 오너는 공식 통합계정 동의 후 차량 상태·7종 경고·연결 기록을 확인할 수 있고, 소프트웨어/품질 팀은 `#canary`(SDV 운영 데모)에서 실차 명령 없이 릴리스 진행률과 보호 상태를 읽기 전용으로 검토할 수 있습니다. 홈과 Canary Lab의 공유 버튼은 모바일 공유 시트 또는 복사 링크로 공개 파일럿을 전달합니다.
