...
충전소와 블루핸즈는 위치 권한을 이미 허용한 경우 현재 위치로 자동 검색하고, iOS Safari처럼 권한 조회 API가 없는 브라우저에서도 실제 위치 권한을 요청합니다. 권한을 거부했거나 확인에 실패한 경우에만 기본 지역 결과를 보여주며, 화면의 `내 위치로 다시 찾기` 버튼으로 언제든 재시도할 수 있습니다. 차량 케어의 7종 경고·차량 여권·공유 링크로 바로 이어집니다. 로그인 전에도 위치 기반 충전·정비 탐색은 사용할 수 있어 첫 화면에서 서비스 가치를 확인할 수 있습니다.
